...
def _run_risk_manager(
    ...
    if not isinstance(trade_plan, dict):
        return _failure("BLOCKED", "trade_plan must be a dictionary")

    context = risk_context if isinstance(risk_context, dict) else {}

    signal = _safe_signal(_get_first(trade_plan, "decision", "signal"))
    ...
    symbol = _get_first(trade_plan, "symbol")
    if not isinstance(symbol, str) or not symbol.strip():
        return _failure("BLOCKED", "trade plan symbol is missing")
    symbol = symbol.strip()

    entry = _safe_float(_get_first(trade_plan, "entry", "entry_price"))
    if entry is None or entry <= 0:
        return _failure("BLOCKED", "trade plan entry is invalid")

    account_balance = _safe_float(_get_first(context, "account_balance", "balance"))
    account_equity = _safe_float(_get_first(context, "account_equity", "equity"))
    if account_balance is None:
        return _failure("BLOCKED", "account balance is required by risk management")
    if account_equity is None:
        return _failure("BLOCKED", "account equity is required by risk management")

    starting_balance = _safe_float(_get_first(context, "starting_balance"))
    if starting_balance is None:
        starting_balance = account_balance

    starting_day_balance = _safe_float(_get_first(context, "starting_day_balance"))
    if starting_day_balance is None:
        starting_day_balance = account_balance

    base_risk_percent = _safe_float(_get_first(context, "base_risk_percent", "risk_percent"))
    if base_risk_percent is None:
        base_risk_percent = 1.0

    preferred_lot = _get_first(context, "preferred_lot")

    opportunity_score = _safe_float(
        _get_first(trade_plan, "opportunity_score", "opportunity_quality")
    )
    if opportunity_score is None:
        opportunity_score = 0.0

    max_drawdown_percent = _safe_float(_get_first(context, "max_drawdown_percent"))
    if max_drawdown_percent is None:
        max_drawdown_percent = 20.0

    max_daily_loss_percent = _safe_float(_get_first(context, "max_daily_loss_percent"))
    if max_daily_loss_percent is None:
        max_daily_loss_percent = 5.0

    margin_safety_buffer_percent = _safe_float(
        _get_first(context, "margin_safety_buffer_percent")
    )
    ...
    risk_market_context = {}
    risk_market_context.update(raw_market_context)
    risk_market_context.update(raw_structural_context)
    risk_market_context["market_context"] = raw_market_context
    risk_market_context["structural_context"] = raw_structural_context
    risk_market_context["trade_plan"] = trade_plan

    symbol_info = _get_first(context, "symbol_info")
    if symbol_info is None:
        try:
            ...
            symbol_info = get_symbol_info(symbol.strip())
    ...
    try:
        risk_result = evaluate_risk(
            ...
            margin_safety_buffer_percent=margin_safety_buffer_percent,
    ...
    if not isinstance(risk_result, dict):
        return _failure("BLOCKED", "risk manager returned invalid output")

    risk_authorized = _safe_bool(risk_result.get("risk_authorized"), False)
...
def _run_risk_bridge(
    ...
    try:
        ...
        bridge_result = authorize_for_execution(signal, risk_result)
    except Exception as exc:
        return _failure(
            "BLOCKED",
    ...
    if not _safe_bool(bridge_result.get("execution_allowed"), False):
        return {
            "status": "BLOCKED",
            "execution_allowed": False,
...
def _run_position_check(
    ...
    if isinstance(position_result, dict):
        allowed = _safe_bool(
            position_result.get(
                "position_allowed",
                position_result.get("allowed", position_result.get("authorized")),
            ),
            False,
        )
        if not allowed:
            return _failure(
                "BLOCKED",
                position_result.get("reason", "position check blocked execution"),
    ...
    try:
        ...
        allowed = check_function(symbol, signal, positions)
    except Exception as exc:
        return _failure("BLOCKED", f"position check failed: {exc}")
...
def _run_final_gate(
    ...
    decision = _safe_signal(_get_first(trade_plan, "decision", "signal"))
    ...
    try:
        ...
        return _failure("BLOCKED", f"final gate failed: {exc}")

    if not isinstance(result, dict):
        return _failure("BLOCKED", "final gate returned invalid output")

    passed = result.get("gate") == "PASS" and result.get("allowed") is True
    if not passed:
        return {
            "status": "BLOCKED",
            "execution_allowed": False,
            "reason": result.get("reason", "final execution gate blocked trade"),
            "gate_result": result,
        }
...
def _run_order_builder(
    ...
    gate_result = final_gate_result.get("gate_result", final_gate_result)
    decision = _safe_signal(_get_first(trade_plan, "decision", "signal"))
    risk_result = trade_plan.get("_risk_result")

    if risk_result is None:
        return _failure("BLOCKED", "risk result missing before order builder")
    ...
    try:
        ...
        order = build_order(
            decision,
            risk_result.get("trade_plan", risk_result),
            gate_result,
        )
    except Exception as exc:
        return _failure("BLOCKED", f"order builder failed: {exc}")

    if not isinstance(order, dict):
        return _failure("BLOCKED", "order builder returned invalid output")
...
def _run_executor(
    ...
    try:
        ...
        try:
            result = execute_order(approved_order, gate=gate_result)
        except Exception as exc:
            return _failure("BLOCKED", f"executor failed: {exc}")
    except Exception as exc:
        return _failure("BLOCKED", f"executor failed: {exc}")

    if not isinstance(result, dict):
        return _failure("BLOCKED", "executor returned invalid output")

    return {
        "status": result.get("status", "READY"),
        "execution_allowed": _safe_bool(
            result.get("authorized", result.get("execution_allowed", False)),
            False,
        ),
...
def execute_pipeline(
    ...
    The execute_live parameter controls whether an already-authorized
    Executor result is delegated to the canonical execution pipeline
    connector. It does not bypass any authorization stage.
    """
    ...
    if decision not in SUPPORTED_SIGNALS:
        return _failure("BLOCKED", "invalid trade-plan decision", decision=decision)
    ...
    final_gate_stage = _run_final_gate(
        ...
        risk_result=risk_result,
        position_result=position_stage.get("position_result", position_stage),
    )
    ...
    executor_stage = _run_executor(
        ...
        gate_result=final_gate_stage.get("gate_result", final_gate_stage),
    )
    ...
    # ==============================================================
    #
    # The Executor remains the authorization layer. When execute_live
    # is requested, the already-authorized Executor result is handed to
    # the canonical connector, which delegates to live_executor.py.
    # The pipeline never calls MT5 directly.
    #
    live_stage: Optional[Dict[str, Any]] = None
    final_status = executor_stage.get("status", "READY")
    final_execution_allowed = executor_stage.get("execution_allowed", False)

    if execute_live:
        try:
            connect_executor = _load_symbol(
                "backend.trading_engine.execution.execution_pipeline_connector",
                "connect_executor_to_live_executor",
            )

            live_stage = connect_executor(
                executor_result=executor_stage.get("executor_result", {}),
                gate=final_gate_stage.get("gate_result", final_gate_stage),
            )

            final_status = live_stage.get("status", "BLOCKED")
            final_execution_allowed = _safe_bool(
                live_stage.get("executed", False),
                False,
            )
            ...
            live_stage = _failure(
                "BLOCKED",
                f"live execution connector failed: {exc}",
                live_execution_exception=True,
            )
